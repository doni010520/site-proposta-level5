...
const App = () => {
  ...
  // Quando subir para o servidor, certifique-se de ter o arquivo 'logo-level5.jpg' na pasta 'public'
  // A pasta 'public' fica na raiz do projeto (no mesmo nível do src e package.json)
  const logoUrl = "/logo-level5.jpg"; 
  // Fallback para visualização se a imagem não carregar:
  // const logoUrl = "[https://placehold.co/150x150/FBBF24/0f172a?text=L5](https://placehold.co/150x150/FBBF24/0f172a?text=L5)"; 

  // Configuração da Identidade Visual (TEMA DARK)
  const brandColors = {
    bg: "bg-slate-900", // Fundo bem escuro
    card: "bg-slate-800", // Cartão cinza escuro
    primaryButton: "bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-400 hover:to-orange-400",
    textMain: "text-slate-100", // Texto claro
    textSecondary: "text-slate-400", // Texto secundário
    inputBorder: "border-slate-600 focus:border-yellow-500", // Bordas sutis
    inputBg: "bg-slate-700", // Input escuro
  ...
  const handleSubmit = async (e) => {
    ...
    // Tenta usar a URL configurada no input, senão usa uma padrão (você pode alterar a padrão aqui)
    const targetUrl = webhookUrl || "[https://n8n.seu-servidor.com/webhook/test](https://n8n.seu-servidor.com/webhook/test)"; 
    
    if (!targetUrl.startsWith('http')) {
      setStatus('error');
      setMessage('URL do Webhook inválida. Verifique as configurações.');
      return;
    }
  ...
  return (
    <div className={`min-h-screen ${brandColors.bg} font-sans pb-10 flex flex-col selection:bg-yellow-500 selection:text-black`}>
      ...
      {/* Header com efeito de vidro (Glassmorphism) e fixo no topo */}
            ...
            <div className="relative group">
              <img 
                ...
                onError={(e) => {
                  ...
                  e.target.src = "[https://placehold.co/150x150/FBBF24/0f172a?text=L5](https://placehold.co/150x150/FBBF24/0f172a?text=L5)";
                }}
              />
            </div>
      ...
      <main className="w-full max-w-2xl mx-auto px-4 mt-8 flex-grow">
        ...
        {showSettings && (
          <div className="w-full bg-slate-800 rounded-lg p-4 mb-6 border border-yellow-500/20 shadow-lg animate-in fade-in slide-in-from-top-2">
            <label className="text-xs text-yellow-500 font-bold uppercase mb-1 block">Webhook URL (n8n)</label>
            <input 
              type="text" 
              value={webhookUrl}
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                ...
                <div className="md:col-span-4">
                  ...
                  <input
                    ...
                    value={formData.modulos_quantidade}
                    onChange={handleChange}
                    placeholder="0"
                    className={`w-full p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                  />
                ...
                <div className="md:col-span-8">
                  ...
                  <input
                    required
                    type="text"
                    name="especificacoes_modulo"
                    value={formData.especificacoes_modulo}
                    onChange={handleChange}
                    placeholder="Ex: Jinko 550W Tiger Pro"
                    className={`w-full p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                  />
                </div>
                ...
                <div className="md:col-span-4">
                  ...
                  <input
                    required
                    type="number"
                    name="inversores_quantidade"
                    value={formData.inversores_quantidade}
                    onChange={handleChange}
                    placeholder="0"
                    className={`w-full p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                  />
                ...
                <div className="md:col-span-8">
                  ...
                  <input
                    required
                    type="text"
                    name="especificacoes_inversores"
                    value={formData.especificacoes_inversores}
                    onChange={handleChange}
                    placeholder="Ex: Deye 5kW Híbrido"
                    className={`w-full p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                  />
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      type="number"
                      step="0.01"
                      name="investimento_kit_fotovoltaico"
                      value={formData.investimento_kit_fotovoltaico}
                      onChange={handleChange}
                      className={`w-full pl-10 p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                      placeholder="0.00"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      type="number"
                      step="0.01"
                      name="investimento_mao_de_obra"
                      value={formData.investimento_mao_de_obra}
                      onChange={handleChange}
                      className={`w-full pl-10 p-4 rounded-xl text-white outline-none border transition-all placeholder:text-slate-500 ${brandColors.inputBg} ${brandColors.inputBorder}`}
                      placeholder="0.00"
                    />
            ...
            <div className="pt-6">
              <button
                ...
                type="submit"
                className={`w-full py-4 rounded-xl font-bold text-slate-900 text-lg shadow-lg shadow-yellow-500/10 flex justify-center items-center gap-3 transition-all hover:shadow-yellow-500/20 active:scale-[0.98] ${brandColors.primaryButton} disabled:opacity-70 disabled:cursor-not-allowed`}
              >
            ...
            {/* Feedback */}
            {status === 'success' && (
              <div className="p-4 bg-green-500/10 text-green-400 rounded-xl flex items-center gap-3 border border-green-500/20">
            ...
            {status === 'error' && (
              <div className="p-4 bg-red-500/10 text-red-400 rounded-xl flex items-center gap-3 border border-red-500/20">
        ...
        <div className="mt-8 text-center pb-8">
          <p className="text-slate-500 text-xs">
            © {new Date().getFullYear()} Level 5 Engenharia Elétrica.
          </p>
        </div>
      </main>
    </div>
